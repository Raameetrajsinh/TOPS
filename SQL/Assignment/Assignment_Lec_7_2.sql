...
-- 4. Retrieve cars with prices closest to the average price, rounded to the nearest hundred.
select * 
from cars 
where price between ((select avg(price) from cars)-2500) and ((select avg(price) from cars)+2500); 

-- 5. List cars with prices higher than the average price of cars manufactured in the year 2021.
select * 
from cars 
where price > (select avg(price) from cars where   year = 2021);

-- 6. Find the model of the cheapest car among those with a V8 engine.
select * 
from cars 
where enginetype = 'V8' and price <= (select min(price) from cars where enginetype = 'V8' );
-- select min(price) from cars where enginetype = 'V8';
-- select min(price) from cars;

-- 7. Retrieve brands with more than two car models in the dataset.
select brand,count(*) 
from cars 
group by brand 
having count(*) >2 ;

-- 8. List cars with prices higher than the average price of cars with the same transmission type and fuel type.
select * 
from cars 
where price >= (select avg(price) from cars where fueltype = transmissiontype );

-- 9. Find the brand with the highest total mileage across all models.
select brand,sum(mileage) 
from cars 
group by brand 
order by sum(mileage) desc limit 1;

-- 10. Retrieve cars with prices higher than the average price of cars with a similar number of doors.--d
select * from cars;
...
select * 
from cars 
where price between ((select max(price) from cars)-5000) and ((select max(price) from cars)+5000);
...
select model 
from cars 
where enginetype = 'V6' and mileage <= (select min(mileage) from cars where enginetype = 'V6' );

-- 15. List cars with prices higher than the average price of cars with the same brand and the same transmission type.--d
select * 
from cars 
where price > (select avg(price) from cars group by brand,transmissiontype);

select avg(price) from cars group by brand,transmissiontype;
-- 16. Retrieve the brand with the highest number of doors on average.--d
select * from cars;
...
-- 19. Retrieve brands where all cars have a mileage greater than 10,000.---d
select brand,model 
from cars 
...
select model 
from cars 
where price >=(select max(price) from cars) and enginetype = 'V8' ;