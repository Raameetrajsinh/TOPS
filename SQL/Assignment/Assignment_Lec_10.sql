...
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.LOCATION 
FROM HOTELS AS A JOIN HOTELS AS B
ON A.LOCATION = B.LOCATION AND A.HOTELID < B.HOTELID;

-- Question 2: Hotels with the same contact number
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.CONTACTNUMBER
FROM HOTELS AS A JOIN HOTELS AS B
ON A.CONTACTNUMBER = B.CONTACTNUMBER AND A.HOTELID < B.HOTELID;

-- Question 3: Hotels with similar price per night.
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.PRICEPERNIGHT
FROM HOTELS AS A JOIN HOTELS AS B
ON A.PRICEPERNIGHT = B.PRICEPERNIGHT AND A.HOTELID < B.HOTELID;

-- Question 4: Hotels with the same amenities and check-in time
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.AMENITIES , A.CHECKINTIME
FROM HOTELS AS A JOIN HOTELS AS B
ON A.AMENITIES = B.AMENITIES AND A.CHECKINTIME = B.CHECKINTIME AND A.HOTELID < B.HOTELID;

-- Question 5: Retrieve pairs of hotels with the same rating.
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.RATING
FROM HOTELS AS A JOIN HOTELS AS B
ON A.RATING = B.RATING AND A.HOTELID < B.HOTELID;

-- Question 6: Find hotels with similar numbers of rooms.
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.NUMROOMS
FROM HOTELS AS A JOIN HOTELS AS B
ON A.NUMROOMS = B.NUMROOMS AND A.HOTELID < B.HOTELID;

-- Question 7: Identify hotels with the same check-out time.
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.LOCATION 
FROM HOTELS AS A JOIN HOTELS AS B
ON A.CHECKOUTTIME = B.CHECKOUTTIME AND A.HOTELID < B.HOTELID;
...
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.LOCATION 
FROM HOTELS AS A JOIN HOTELS AS B
ON A.AMENITIES = B.AMENITIES AND A.HOTELID < B.HOTELID;

-- Question 10: Retrieve pairs of hotels with the same rating and review date.
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.RATING , R.REVIEWDATE
FROM HOTELS AS A JOIN HOTELS AS B JOIN HOTELREVIEWS AS R
ON A.RATING = B.RATING AND A.HOTELID = R.HOTELID AND A.HOTELID < B.HOTELID
WHERE B.HOTELID = R.HOTELID;

-- Question 11: Find hotels with the same rating and review rating.
SELECT A.HOTELID, A.HOTELNAME ,B.HOTELID, B.HOTELNAME, A.RATING , R.RATING
FROM HOTELS AS A JOIN HOTELS AS B JOIN HOTELREVIEWS AS R
ON A.RATING = B.RATING AND A.HOTELID = R.HOTELID AND A.HOTELID < B.HOTELID
WHERE B.HOTELID = R.HOTELID;
